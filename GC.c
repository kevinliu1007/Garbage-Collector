...
void _mark(void * ptr) {
	size_t * temp = is_pointer((size_t *)ptr);
	if(temp == NULL) return;
	if(is_marked(temp)) return;
	
	mark(temp);
	
	size_t * size = next_chunk(temp);
	size_t * temp2 = temp + 1;
	for(;temp2 < size;temp2 = temp2 + 1) {
		_mark(temp2);
	}
}

void walk_region_and_mark(void* start, void* end) {
	size_t * temp = (size_t *)start;
	while(temp < (size_t *)end) {
		_mark(temp);
		temp = temp + 1;
	}	
}
